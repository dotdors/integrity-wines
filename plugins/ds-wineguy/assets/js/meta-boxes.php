...
function dswg_wine_files_callback($post) {
    $files_raw = get_post_meta($post->ID, 'dswg_wine_files', true);
    // Meta is stored as a comma-separated string of attachment IDs
    if (is_array($files_raw)) {
        $files = array_filter($files_raw);
    } elseif (!empty($files_raw)) {
        $files = array_filter(explode(',', $files_raw));
    } else {
        $files = [];
    }
